...
bool verifyEH(const char *hdr, const char *soln, int n, int k){
    ...
    blake2b_update(&state, (unsigned char*)&hdr[0], 140/*528*/);

    std::vector<unsigned char> minimal(soln, soln + 1344/*1408*/);
...
template int Equihash<200,9>::InitialiseState(blake2b_state *base_state);
